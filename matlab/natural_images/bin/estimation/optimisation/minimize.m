function [X, fX, i] = minimize(X, fun, length, varargin)

% Minimize a differentiable multivariate function. 
%
% Usage: [X, fX, i] = minimize(X, fun, length, P1, P2, P3, ... )
%
% where the starting point is given by "X" (D by 1), and the function as a 
% handle "fun", must return a function value and a vector of partial
...
% to 1.0). The parameters P1, P2, P3, ... are passed on to the function fun.
%
...
% the implementation of your "fun" function). The function returns the found
...
% 1) we first enter a while loop
% which uses point 1 (p1) and (p2) to compute an extrapolation (p3), until we
% have extrapolated far enough (Wolfe-Powell conditions). 
% 
% 2) if necessary, we  enter the second loop which takes p2, p3 and p4 chooses the subinterval
% containing a (local) minimum, and interpolates it, unil an acceptable point
% is found (Wolfe-Powell conditions). Note, that points are always maintained
% in order p0 <= p1 <= p2 < p3 < p4. 
%
% 3) compute a new search direction using
...
[f0 df0] = fun(X);          % get function value and gradient
fX = f0;
