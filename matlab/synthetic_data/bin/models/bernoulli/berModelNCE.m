function [ out1, varargout ] = berModelNCE( X, theta, varargin )
...
if (outputflag ~= 1 && outputflag ~= 2)
	logPhi = zeros(N, 1);
	logPhi(Ifalse) = log(theta(1)) - theta(3);
	logPhi(Itrue) = log(theta(2)) - theta(3);
end

if (nargout > 1 || outputflag == 1)
    % Calculate log-gradient w.r.t theta
	logGrad_t = zeros(N, M);
	logGrad_t(Ifalse, :) = [repmat([1/theta(1), 0], Nfalse, 1), -ones(Nfalse, 1)];
	logGrad_t(Itrue, :) = [repmat([0, 1/theta(2)], Ntrue, 1), -ones(Ntrue, 1)];
